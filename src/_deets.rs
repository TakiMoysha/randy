...
fn get_procs_count(proc_stat: &Vec<String>) -> String {
    return match proc_stat
        .iter()
        .find(|line| line.starts_with("procs_running"))
    {
...
pub fn get_cpu_mhz() -> Vec<u16> {
    ...
    return try_match_strings_from_file(&mut *file, &vec!["cpu MHz"])
        .unwrap()
        .into_iter()
        .map(|s| {
...
fn get_ps_from_proc(counter: u64, mod_top: u64, mem_used: f64) -> Vec<PsInfo> {
    ...
    fn _do_cpu(path: &str, pid: &str, total_time: f64) -> f32 {
        ...
        let proc_time: f64 =
            stat_vec[13].parse::<f64>().unwrap() + stat_vec[14].parse::<f64>().unwrap();
    ...
    fs::read_dir("/proc").unwrap().for_each(|dir_entry| {
        ...
        if path.bytes().nth(6).unwrap().is_ascii_digit() {
            ...
            let proc_used = status_lines[1][7..(status_lines[1].len() - 3)]
                .trim()
                .parse::<f64>();
    ...
    if should_run_retain {
        PROC_STAT_READERS
            .lock()
            .unwrap()
            .retain(|i, _| pids.contains(&i.to_string()));
...
fn get_cpu_voltage_rpi() -> String {
    let output = match Command::new("vcgencmd")
        .arg("measure_volts")
        .arg("core")
        .output()
    {
...
pub fn do_func(item: &ConfigString, frame_cache: &FrameCache) -> String {
    ...
    let ret: String = match func {
        "hostname" => timings!(
            func,
            get_hostname_from_utsname,
            frame_cache.utsname.nodename as [c_char; 65]
        ),
        ...
        #[cfg(feature = "nvidia")]
        "nvidia_gpu_temp" => timings!(
            "nvidia_temp",
            get_nvidia_gpu_temp,
            item["idx"].as_i64().unwrap() as u32
        ),
    ...
    lines.iter().find(|line| {
        ...
        keys.iter().find(|path| {
            if tokens[1] == **path {
                ...
                let free = ((statvfs.f_bsize as f64 / 1024.0) * (statvfs.f_bfree as f64 / 1024.0))
                    / 1024.0;
                let mut total = ((statvfs.f_frsize as f64 / 1024.0)
                    * (statvfs.f_blocks as f64 / 1024.0))
                    / 1024.0;
...
pub fn get_frame_cache(counter: u64, mod_top: u64, do_top_bool: bool) -> FrameCache {
    ...
    #[cfg(feature = "timings")]
    println!(
        "Size of PROC_PID_FILES: {}",
        PROC_PID_FILES.lock().unwrap().len()
    );
    #[cfg(feature = "timings")]
    println!(
        "Size of PROC_STAT_READERS: {}\n",
...
fn get_net_dev() -> HashMap<String, (u64, u64)> {
    ...
    lines.iter().skip(2).for_each(|line| {
        let tokens = split_spc_to_strs!(line);
        map.insert(
            String::from(&tokens[0][0..(tokens[0].len() - 1)]),
            (
